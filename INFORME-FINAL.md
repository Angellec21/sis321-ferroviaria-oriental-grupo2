...
El mayor aprendizaje no técnico del proyecto fue el proceso de despliegue en Render. Localmente el sistema funcionaba perfectamente, pero al subir a producción aparecieron problemas que no habíamos anticipado: el esquema `dw` no existía en la base de datos de Render y el script `init.js` fallaba por dependencias de llaves foráneas en el orden de creación de tablas. La solución fue abandonar ese enfoque y usar `pg_dump` directamente en la base de datos local para exportar un dump completo de 3788 líneas, y restaurarlo con `psql` al External URL de Render. Este proceso tomó varias horas de diagnóstico.

Otros problemas que surgieron al subir a producción: (1) **CORS roto** — la variable `CORS_ORIGIN=*` al ser procesada con `.split(',')` generaba `['*']` en vez de `'*'`, lo que bloqueaba silenciosamente todas las peticiones del frontend; el login parecía funcionar visualmente pero no hacía ninguna llamada real al backend. (2) **"Not Found" al recargar cualquier ruta** — React Router maneja la navegación en el cliente, pero Render desconocía esto y devolvía 404 al acceder directamente a `/ventas` o `/dashboard`; se resolvió con un archivo `_redirects` en `frontend/public/` con la regla `/* /index.html 200`. (3) **Redis URL con formato incorrecto** — el código esperaba un objeto `{host, port}` pero Render entrega una cadena de texto `redis://...`, lo que requirió modificar `redis.js` para detectar el formato.

Si tuviéramos más tiempo, prepararíamos un checklist de producción antes de cada despliegue que incluya: verificar variables de entorno, probar CORS con origen real, y validar que todas las rutas SPA funcionen con recarga directa. También agregaríamos tests automatizados con Jest que detecten estos problemas antes de llegar a producción, en lugar de descubrirlos en vivo.
